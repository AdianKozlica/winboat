...
set RDP_SESSION_CHECK_SCRIPT_PATH=%INSTALL_DIR%\scripts\check_rdp_session.ps1
...
reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Run" /v "RdpSessionCheck" /t REG_SZ /d "conhost --headless powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"%RDP_SESSION_CHECK_SCRIPT_PATH%\"" /f

:: Misc
reg add "HKLM\Software\WinBoatVncCheck" /v Checked /t REG_DWORD /d 0
